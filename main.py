...
def main_threads():
    ...
    matchs_details, links_to_matchs, links_to_teams = pool.starmap(main_parser.main_parse, zip(ids_list))
...
def main_pool():
    ...
    # matchs_details, links_to_matchs, links_to_teams = pool.starmap(main_parser.main_parse, zip(ids_list))
    result = pool.starmap(main_parser.main_parse, zip(ids_list))

    for res in result:
        matchs_details.extend(res[0])
        links_to_matchs.extend(res[1])
        links_to_teams.extend(res[2])
    
    
    links_to_teams = list(split(links_to_teams, 4))
    
    
    result = pool.starmap(team_parser.team_parse, zip(links_to_teams))

    links_to_teams = []
    
    for links in result:
        links_to_teams.extend(links)
    
    excel_saver.save_xlsx_match_details(matchs_details)
    excel_saver.save_xlsx_urls(links_to_matchs)
    excel_saver.save_xlsx_teams(links_to_teams)

    excel_saver.save_xlsx_match_details(matchs_details, 'matches')
    excel_saver.save_xlsx_urls(links_to_matchs, 'urls')
    excel_saver.save_xlsx_teams(links_to_teams, 'teams')
